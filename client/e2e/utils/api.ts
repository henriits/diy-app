...
type UserLogin = Parameters<typeof trpc.users.login.mutate>[0]
...
export async function loginNewUser(userLogin: UserLogin = fakeUser()): Promise<UserLoginAuthed> {
  try {
    await trpc.users.signup.mutate(userLogin)
  } catch (error) {
    // ignore cases when user already exists
  }

  const loginResponse = await trpc.users.login.mutate(userLogin)
